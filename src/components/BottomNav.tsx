...
const BottomNav = () => {
  ...
  const moreItems = [
    ...
    { name: 'Complete', path: '/complete', icon: faCircleCheck },
  ];
    ...
    <>
      ...
      {isMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-2050 flex items-end justify-center p-4">
          ...
          <div className="relative -top-15 w-full max-w-sm bg-[#161618] rounded-4xl border border-white/10 shadow-2xl overflow-hidden p-6 animate-in slide-in-from-bottom-10 duration-300">
            ...
            <div className="grid grid-cols-3 gap-4">
              {moreItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/5 hover:bg-white/10 transition-all border border-transparent hover:border-(--primary)/20"
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isActive(item.path) ? 'bg-(--primary)/20 text-(--primary)' : 'bg-white/5 text-white/40'}`}>
                    <FontAwesomeIcon icon={item.icon} />
                  </div>
                  <span className={`text-[10px] font-bold ${isActive(item.path) ? 'text-(--primary)' : 'text-white/40'}`}>{item.name}</span>
                </Link>
              ))}
            </div>
          </div>
