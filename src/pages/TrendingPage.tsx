...
const TrendingPage = () => {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        setLoading(true);
        const res = await authenticatedFetch(`${ANIMEPLAY_API_BASE_URL}/trending?page=${page}`);
        const json = await res.json();
        
        if (json.status === 'success' && json.data?.data) {
          setAllTrendingList(mapApiData(json.data.data));
  ...
  if (loading) return <Loader message="Memuat anime populer..." />;
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-12 pb-20">
      ...
      <section>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
          {allTrendingList.map((anime, index) => (
            <div key={anime.id} className="animate-reveal" style={{ animationDelay: `${index * 0.02}s` }}>
