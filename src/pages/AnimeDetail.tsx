...
const AnimeDetail = () => {
        ...
        const fetchDetail = async () => {
            ...
            try {
                const res = await authenticatedFetch(
                    `${ANIMEPLAY_API_BASE_URL}/detail/${slug}`,
                );
                ...
                if (json.status === "success" && json.data) {
                    ...
                    const detailed: DetailedAnime = {
                        ...
                        thumbnail: d.thumb || d.image || d.image_url || d.thumbnail || d.poster,
                        ...
                        recommended:
                            d.recommendationList?.map((r: any) => ({
                                animeId: r.seriesSlug || r.slug || r.id || r.animeId,
                                title: r.anime_title || r.title,
                                poster: r.thumb || r.image || r.image_url || r.thumbnail || r.poster,
                            })) || [],
                    };
    ...
    return (
        <div className="min-h-screen bg-(--bg-dark) -mt-20">
            ...
            {/* Immersive Header */}{" "}
            <div className="relative w-full min-h-[75vh] h-auto md:h-[80vh] flex items-start lg:items-end">
                {/* Background Banner */}
                            ...
                            <div className="flex-1 space-y-4 w-full">
                                ...
                                <div className="flex flex-wrap justify-center lg:justify-start gap-2">
                                    {anime.genre.map((g, i) => (
                                        <span
                                            key={i}
                                            className="text-sm text-white/40 hover:text-(--primary) cursor-default transition-colors"
                                        >
                                            {g}
                                            {i < anime.genre.length - 1
                                                ? " • "
                                                : ""}
                                        </span>
                                    ))}
                                ...
                                <div className="flex items-center justify-center lg:justify-start gap-4 pt-4">
                                    <button
                                        onClick={() =>
                                            ...
                                            handleWatch(anime.episodes[0].slug)
                                        }
                            ...
                            {filteredEpisodes.map((ep) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-white/90 truncate group-hover:text-(--primary) transition-colors">
                                            Episode {ep.episode}
                                        </h4>
                                    ...
                                    <FontAwesomeIcon
                                        icon={faPlay}
                                        className="text-(--primary) opacity-0 group-hover:opacity-100 transition-opacity text-xs"
                                    />
                                </button>
                            ))}
                        </div>
                    ...
                    <section className="bg-white/5 rounded-2xl p-6 border border-white/5 space-y-6">
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="space-y-2">
                                ...
                                <span className="text-white/80 text-xs leading-relaxed block">
                                    {anime.info?.japanese}
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {anime.recommended.slice(0, 4).map((rel) => (
                                    ...
                                    >
                                        ...
                                        <h4 className="text-[11px] font-medium line-clamp-2 text-white/70 group-hover:text-(--primary) transition-colors">
                                            {rel.title}
                                        </h4>
                                    </Link>
                                ))}
